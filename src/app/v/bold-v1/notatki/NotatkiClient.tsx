...
function NotatkiContent({ initialNotes, initialIdeas }: Props) {
  ...
  return (
    <div className="bv1-form-screen">
          ...
          <div className="bv1-radio-row">
            {([
              ...
              <button
                key={opt.val}
                type="button"
                className={filterKind === opt.val ? "bv1-radio-btn is-active" : "bv1-radio-btn"}
      ...
      <section className="bv1-form-section">
        {filtered.length === 0 ? (
          <p style={{ color: "var(--bv1-text-muted)", textAlign: "center", padding: 24 }}>
          ...
          <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 12 }}>
            {filtered.map((item) => (
              <li
                ...
                style={{
                  padding: 16,
                  background: "var(--bv1-bg-card-warm)",
                  border: "1px solid var(--bv1-border-subtle)",
                  borderLeft: `3px solid ${item.kind === "note" ? "var(--bv1-accent-red)" : "var(--bv1-accent-yellow)"}`,
                  borderRadius: "var(--bv1-radius-md)",
                }}
              >
                {editingNoteId === item.id ? (
                  ...
                  <>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 12, marginBottom: 8 }}>
                      <h3 style={{ margin: 0, fontSize: 16, fontWeight: 600, color: "var(--bv1-text-on-card)" }}>
                        {item.title}
                      </h3>
                      <span
                        style={{
                          ...
                          background: item.kind === "note" ? "var(--bv1-accent-red)" : "var(--bv1-accent-yellow)",
                          color: item.kind === "note" ? "var(--bv1-text-on-deep)" : "var(--bv1-text-on-card)",
                          fontWeight: 600,
                          flexShrink: 0,
                        }}
                      >
                    ...
                    {item.body && (
                      <p style={{ margin: "0 0 8px", color: "var(--bv1-text-on-card)", fontSize: 14, whiteSpace: "pre-wrap" }}>
                    ...
                    <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                      ...
                      <span style={{ marginLeft: "auto", fontSize: 11, color: "var(--bv1-text-muted)" }}>
      ...
      {showAdd && (
        ...
        >
          <div
            ...
            style={{
              background: "var(--bv1-bg-card)",
              borderRadius: "var(--bv1-radius-lg)",
              borderTop: "4px solid var(--bv1-accent-red)",
              padding: 32,
              maxWidth: 560,
              width: "100%",
              maxHeight: "90vh",
              overflowY: "auto",
              boxShadow: "var(--bv1-shadow-elevated)",
            }}
          >
            <h2 className="bv1-form-section-title" style={{ marginBottom: 16 }}>
              Nowy element
            </h2>

            <div className="bv1-radio-row" style={{ marginBottom: 16 }}>
              {(["note", "idea"] as const).map((kind) => (
                <button
                  key={kind}
                  type="button"
                  className={newKind === kind ? "bv1-radio-btn is-active" : "bv1-radio-btn"}
            ...
            {newKind === "note" && (
              <div style={{ marginBottom: 12 }}>
                <label className="bv1-form-label">Tytuł</label>
                <Input
                  value={newTitle}
            ...
            <div style={{ marginBottom: 12 }}>
              <label className="bv1-form-label">
            ...
            <div style={{ marginBottom: 16 }}>
              <label className="bv1-form-label">Tagi</label>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {IDEA_TAGS.map((tag) => (
                  <button
                    ...
                    className={newSelectedTags.includes(tag) ? "bv1-radio-btn is-active" : "bv1-radio-btn"}
...
export function BoldV1NotatkiClient(props: Props) {
  return (
